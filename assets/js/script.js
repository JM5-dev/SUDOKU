...
class SudokuGame {
    ...
    setupEventListeners() {
        ...
        // Usa window per catturare eventi globali indipendentemente dal focus
    ...
    generateNewGame() {
        ...
        // Rimuovi active dal pulsante nota e ripristina il pulsante pausa
        document.getElementById('note-mode').classList.remove('active');
        const pauseBtn = document.getElementById('pause-btn');
        pauseBtn.textContent = '⏸️';
        pauseBtn.title = 'Pausa';
        
        // Resetta il filtro blur dal gioco precedente
    ...
    // Rimuove un numero casuale di celle per creare il puzzle, verificando risolvibilità
    removeNumbers(count) {
        let removed = 0;
        const cells = [];
        
        // Crea una lista di tutte le celle non vuote
        for (let row = 0; row < 9; row++) {
            for (let col = 0; col < 9; col++) {
                if (this.board[row][col] !== 0) {
                    cells.push({row, col});
        ...
        // Rimuovi numeri verificando che il puzzle rimanga risolvibile
        for (let cell of cells) {
            ...
            // Verifica che il puzzle abbia esattamente una soluzione
            if (this.countSolutions(this.board) === 1) {
                removed++;
            } else {
                // Se il puzzle non è più risolvibile o ha più soluzioni, ripristina
                this.board[row][col] = backup;
            }
        }
    }
    
    // Conta il numero di soluzioni possibili per il puzzle (massimo 2)
    countSolutions(board) {
        let solutionCount = 0;
        
        const backtrack = () => {
            // Se abbiamo trovato più di una soluzione, interrompi la ricerca
            ...
            // Se raggiungiamo qui, abbiamo una soluzione completa
            solutionCount++;
        };
        
        // Crea una copia temporanea della board per la ricerca
        const tempBoard = board.map(row => [...row]);
        this.countSolutionHelper(tempBoard, (count) => {
            solutionCount = count;
        });
    ...
    // Helper per contare le soluzioni in modo efficiente
    countSolutionHelper(board, callback) {
        let solutionCount = 0;
        
        const solve = () => {
            if (solutionCount > 1) return; // Interrompi se trovate più di 1 soluzione
    ...
    renderBoard() {
        ...
        if (previousSelection) {
            // Cerca lo specifico input editabile della cella
            const input = document.querySelector(`input.cell-input[data-row="${previousSelection.row}"][data-col="${previousSelection.col}"]`);
            if (input) {
    ...
    handleInput(e) {
        ...
        // Non elaborare se la cella non è vuota (comportamento identico ai numeri virtuali)
        if (this.board[row][col] !== 0) {
            e.target.value = '';
            return;
        }
    ...
    // Gestisce i tasti premuti (numeri, frecce direzionali, delete)
    handleKeyPress(e) {
        // Gestisci tasto N per attivare/disattivare la modalità nota (funziona sempre, tranne in pausa)
        if (e.key === 'n' || e.key === 'N') {
            e.preventDefault();
            this.toggleNoteMode();
            return;
        ...
        // Gestisci tasto S per suggerimento (funziona sempre, tranne in pausa)
        if (e.key === 's' || e.key === 'S') {
            e.preventDefault();
            this.giveHint();
            return;
        ...
        // Gestisci numeri 1-9 (permette di inserire numeri anche dopo aver cliccato i pulsanti)
        if (/^[1-9]$/.test(e.key)) {
            const activeElement = document.activeElement;
            // Se il focus è già su un input della cella, lascia che l'evento 'input' lo gestisca
        ...
        // Gestisci Delete per cancellare la cella
        if (e.key === 'Delete') {
            e.preventDefault();
        ...
        // Gestisci Backspace per annullare l'ultima mossa
        if (e.key === 'Backspace') {
            e.preventDefault();
            this.undoMove();
            return;
    ...
    giveHint() {
        ...
        // Evidenzia la cella con il suggerimento per 5 secondi
        const hintCell = document.querySelector(`[data-row="${randomCell.row}"][data-col="${randomCell.col}"]`).closest('.cell');
        if (hintCell) {
